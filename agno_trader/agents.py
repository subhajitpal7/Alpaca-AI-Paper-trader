...
def build_trading_team(gemini_model_id: str = "gemini-2.5-flash") -> Team:
    ...
    researcher = Agent(
        name="Researcher",
        role="Fetch latest price and recent history for a given ticker and also the account overview.",
    ...
    team = Team(
        ...
        model=Gemini(id=gemini_model_id),
        instructions=[
            "Researcher: fetch  and corresponding account details of the user when asked and return structured JSON {symbol, price, time,{ account overview: json}}.",
            "Strategist: always call the portfolio overview tool first to get account state and explain the details. Then, based on the Researcher's data and portfolio state, propose ONE decision in strict JSON form: {action: 'buy'|'sell'|'hold', symbol: str, qty: int, reason: str, confidence: float (0-1)}.",
